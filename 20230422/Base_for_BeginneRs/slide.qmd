...
-   多くの便利なパッケージは{base}な文法で記述
    -   初心者を抜け出したらきっと関数の中身を読むと思う
-   tidyverseで関数を作ろうとすると、ちょっとめんどくさい配慮がいる。
    -   自分で関数を作る場合にも`base`で書いたほうが楽

## 御託はここまで

-   「統計学」つってるけど多変量解析も検定もないです。
-   時間もそんなないので全部は見ない。トピックだけ。
...
colnames(usedata) # names of columns
```
...
```{r echo = TRUE}
plot(x = 1/usedata$distance.to.store, y = usedata$store.spend,
...
## まとめ
- `tidyverse`を使わなくても(割と)できることがある
  - この書き方に慣れると、いろんなヘルプが読めるようになる
